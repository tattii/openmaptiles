...
RETURNS TABLE(osm_id bigint, geometry geometry, name text, name_en text, name_de text, tags hstore, place city_place, "rank" int, capital int) AS $$
      ...
      row_number() OVER (
        ...
        WHERE geometry && bbox
          AND ((zoom_level = 7 AND place <= 'town'::city_place
            OR (zoom_level BETWEEN 8 AND 9 AND place <= 'village'::city_place)
            OR (zoom_level BETWEEN 10 AND 11 AND place <= 'suburb'::city_place)
            OR (zoom_level BETWEEN 12 AND 13 AND place <= 'hamlet'::city_place)
            OR (zoom_level >= 14)
          ))
    ) AS ranked_places
    WHERE (zoom_level BETWEEN 7 AND 8 AND (gridrank <= 4 OR "rank" IS NOT NULL))
       ...
       OR (zoom_level BETWEEN 11 AND 12 AND (place = 'suburb'::city_place OR gridrank <= 14 OR "rank" IS NOT NULL))
       OR (zoom_level >= 13);
