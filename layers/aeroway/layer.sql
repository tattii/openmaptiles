...
RETURNS TABLE(geometry geometry, class text, name text) AS $$
    SELECT geometry, aeroway AS class, name FROM (
        ...
        SELECT geometry, aeroway, ''::text AS name
        FROM osm_aeroway_linestring WHERE zoom_level >= 11
        UNION ALL
        -- etldoc:  osm_aeroway_polygon_gen2 -> layer_aeroway:z12
        SELECT geometry, aeroway, ''::text AS name
        FROM osm_aeroway_polygon_gen2 WHERE zoom_level BETWEEN 11 AND 12
        UNION ALL
        ...
        SELECT geometry, aeroway, name
        FROM osm_aeroway_airport WHERE zoom_level >= 11
    ) AS zoom_levels
    WHERE geometry && bbox;
